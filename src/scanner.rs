...
mod tests {
    ...
    fn discovers_files_and_populates_manifest() {
        ...
        let files =
            discover_files(&session_root, &cache_path, None, None, true).expect("discover files");
    ...
    fn refreshes_manifest_file_metadata_for_past_directories() {
        ...
        let first =
            discover_files(&session_root, &cache_path, None, None, true).expect("first discover");
        thread::sleep(Duration::from_millis(10));
        fs::write(&file_path, "{\"a\":1}\n{\"b\":2}\n").expect("rewrite file");

        let second =
            discover_files(&session_root, &cache_path, None, None, true).expect("second discover");
